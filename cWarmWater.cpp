...
void cWarmWater::Control(void) {
	// If Warm Water is needed, execute the pump with the static power plus power determined by PID
	// controller. Else switch PID to manual and stop pump.
	WarmWaterFlow = analogRead(PinWarmWaterFlow) / 1023.0 * Vcc * 76 / 6 -
					26 / 6;  // WarmWaterFlow determined by Sensor converted to Liters/min

	double Factor1 = (0.6 - 1) / (75 - 50) * (IsTempBoilerTop.get() - 50) +
					 1;  // Consideration of Heat Exchanger Behaviour at higher Boiler Temperatures
...
void cWarmWater::getSP(JsonObject& root) { root["WTs"] = SpTemp; }
